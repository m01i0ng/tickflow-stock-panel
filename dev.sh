...
if [ ! -d "$BACKEND_DIR/.venv" ] || [ "${#BACKEND_EXTRA_ARGS[@]}" -gt 0 ]; then
  ...
  # bash 3.2 兼容:set -u 下展开空数组 "${ARR[@]}" 会报 unbound,用 + 守卫兜底
  # --frozen: 严格按 uv.lock 安装, 若 pyproject.toml 与 lock 漂移则明确报错, 不静默重写锁定文件
...
if [ ! -d "$FRONTEND_DIR/node_modules" ]; then
  ...
  # --frozen-lockfile: 严格按 pnpm-lock.yaml 安装, package.json 漂移时报错而非改写锁
