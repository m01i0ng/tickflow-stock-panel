...
- **缠论结构分析**(指数页;可选依赖 `czsc`,启用方式 `uv sync --extra chan`,Docker 构建传 `BACKEND_EXTRAS=chan`):仅指数,个股/ETF 接口 fail-closed。多级别(日线 / 1F / 5F / 10F / 15F / 30F / 60F / 120F)分型、笔(区分已确认/未确认)、中枢(ZG/ZD 跨度线 + 矩形框)叠加渲染 + 最新信号。日线级读指数 enriched(不复权);分钟级由 `kline_index_minute` 的 1m 按 A 股时段合成,级别按需加载。点击分钟级别且本地无 1m 时自动按 TickFlow 单次上限(`klines` count max 10000 根,约 41 交易日)同步当前指数。日期优先用指数页配置的起止; 窗口内根数不足则退回可得最大条数(最多 10000 根)。分钟级优先拉 TickFlow 原生周期(1m/5m/10m/15m/30m/60m), 120分钟由 60分钟合成。指数日线盘中跟随行情缓存最后一根,不要求先配指数监控规则。**盘中自动刷新**:A 股交易时段每 30s 轮询已加载级别,后端按最后一根 bar 指纹自动失效重算。未安装依赖时接口明确返回"未启用",不影响主流程。缠论为分析展示能力,其涨停分类与信号不作为选股/监控的裁决口径。
